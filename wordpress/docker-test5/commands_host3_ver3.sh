targethost="docker-test5-wordpress3-1"
zurl="http://ru.domain.local"
docker exec -it $targethost bash -c "wp --info"
docker exec -it $targethost bash -c "wp core config --dbname=wordpress --dbuser=wordpress --dbpass=your_wordpress_password --dbhost=db --dbprefix=wp_ --path=/var/www/html --allow-root"
docker exec -it $targethost bash -c "wp core install --url=$zurl --title='Just Another Blog' --admin_user=admin --admin_password=P@ssw0rd --admin_email=admin@domain.local --path=/var/www/html --allow-root --skip-email"
